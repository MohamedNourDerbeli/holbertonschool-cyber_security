...
Features:
    - Only modifies heap memory
    - Safe length validation
    - Permission checking
    - ASCII string validation

Examples:
    sudo ./read_write_heap.py 1234 password secret
    sudo ./read_write_heap.py $(pgrep myprogram) "old text" "new text"

Note:
    Requires root privileges to access another process's memory.
"""
...
def print_error(message):
    ...
    Returns:
        None: Exits program with status code 1
    """
...
def read_write_heap(pid, search_str, replace_str):
    ...
    Args:
        pid (int): Process ID of target process
        search_str (str): ASCII string to search for
        replace_str (str): ASCII string to replace with
        
    Returns:
        None: Modifies process memory directly
        
    Raises:
        IOError: If memory access fails
        PermissionError: If insufficient privileges
        ValueError: If string validation fails
        
    Process:
        1. Locates heap memory region
        2. Searches for all occurrences of search_str
        3. Validates replacement safety
        4. Performs in-memory replacement
    """
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 4:
        ...
        print(__doc__)  # Show module documentation
        sys.exit(1)
...
1. Basic Functionality:
   - Target process with known string in heap
   - Verify replacement occurs
   - Example: Replace "Holberton" with "School"

2. Edge Cases:
   - Non-existent PID
   - Invalid PID (negative number, non-numeric)
   - Search string not found
   - Replacement string longer than search string
   - Non-ASCII strings
   - Insufficient permissions

3. Safety Tests:
   - Attempt to replace across heap boundaries
   - Very large heap regions
...
- Create test processes with known heap contents
- Use memory inspection tools to verify changes
- Test on different Python versions
- Consider edge cases in string matching
"""
